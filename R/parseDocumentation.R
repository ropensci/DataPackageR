.parseDocumentation <- function(all_r_files) {
    ...
    indx <- lapply(lapply(docs, function(x) lapply(x, function(y) sum(grepl("#'", 
        y)))), function(x) unlist(x, use.names = FALSE) > 0)
    docs <- lapply(1:length(docs), function(j) docs[[j]][indx[[j]]])
    # Extract @name
    doc_names <- lapply(docs, function(x) unlist(lapply(x, function(y) gsub(".+@name (\\D+)", 
        "\\1", {
            y[grepl("@name", y)]
        })), use.names = FALSE))
