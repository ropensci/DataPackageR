...
dataVersion <- function(pkg, lib.loc = NULL) {
    res <- suppressWarnings(packageDescription(pkg, lib.loc = lib.loc, fields = "DataVersion"))
    if (!is.na(res)) 
        package_version(res) else stop(gettextf("package %s not found or has no DataVersion string", sQuote(pkg)), 
        domain = NA)
}
