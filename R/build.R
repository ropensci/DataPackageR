...
buildDataSetPackage <- function(packageName = NULL,vignettes=FALSE,masterfile=NULL) {
  ...
  if (is.null(packageName)) {
    ...
    #Otherwise is_r_package matches the proposed directory and we know it has a DESCRIPTION file so we're goot to go.
  ...
  #This should always be a proper name of a directory, either current or a subdirectory
  packageName = basename(package_path)
  if(inherits(try(is_r_package$find_file(path=package_path)),"try-error")){
    flog.fatal(paste0(package_path, " is not a valid R package directory beneath ", getwd()))
