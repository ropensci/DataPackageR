...
.saveSessionInfo <- function() {
  ...
  save(sessionInfo, 
    file = file.path(project_extdata_path(), 
                     "sessionInfo.RData"))
}

#' @importFrom containerit Cmd dockerfile
.buildCommand <- function() {
  cmd <- containerit::Cmd(exec = "/bin/bash",
      params = c("-c", 
            "Rscript -e 'DataPackageR::package_build()';cp ../*.tar.gz /host"))
  cmd
}
...
.buildDockerFile <- function(exclude = NULL) {
  ...
  warns <- testthat::capture_warnings(
    df <- dockerfile(from = project_extdata_path("sessionInfo.RData"),
                     copy = "script_dir", cmd = cmd))
  ...
  if (length(warns) != 0) {
    cat(crayon::inverse(
      "These packages should be installed from GitHub or CRAN\n"))
    .bullet(crayon::inverse(warns), bullet = "\u274c")
  }
  df@instructions[[1]]@commands[2] <- 
    paste0(df@instructions[[1]]@commands[2], " \\\n\tzlib1g-dev")
  return(df)
}
