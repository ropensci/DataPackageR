...
DataPackageR <- function(arg = NULL, masterfile = NULL) {
    ...
    # This is cruft.  if (is.null(arg)) { parser <- OptionParser(usage = 'R CMD
    # DataPackageR [options] package') arguments =
    # parse_args(parser,positional_arguments = 1) opt = arguments$options pkg_dir <-
    # arguments$args }else{ if (arg == './') { flog.fatal('Don't can't call
    # buildDataSetPackage() inside a package directory.') stop('Quitting',call. =
    # FALSE) } pkg_dir <- arg }
    pkg_dir = arg
    ...
    if (!file.exists(target)) {
        ...
        old <- getwd()
        # TODO see if we can get rid of the tryCatch tryCatch({ cd into the package
        # directory
        setwd(pkg_dir)
        ...
        # r_files = unlist(map(ymlconf[['configuration']][['files']],'name')) subset to
        ...
        pkg_description <- try(read.description(file = "DESCRIPTION"), silent = TRUE)
        ...
        for (i in seq_along(r_files)) {
            ...
            flog.info(paste0(sum(objectsToKeep %in% object_names), " required data objects created by ", 
                basename(r_files[i])))
            
            if (sum(objectsToKeep %in% object_names) > 0) {
                for (o in objectsToKeep[objectsToKeep %in% object_names]) assign(o, 
                  get(o, dataEnv), ENVS)
            }
        }
        dataEnv = ENVS
        ...
        if (!is.null(old_data_digest)) {
            ...
            if (.compare_digests(old_data_digest, new_data_digest, delta = masterfile) & 
                ...
                can_write <- TRUE
                flog.info(paste0("Processed data sets match existing data sets at version ", 
                ...
                can_write <- TRUE
                flog.info(paste0("Data has been updated and DataVersion string incremented automatically to ", 
                  new_data_digest$DataVersion))
            } else if (.compare_digests(old_data_digest, new_data_digest, delta = masterfile) & 
            ...
            if (can_write) {
                .save_data(new_data_digest, pkg_description, ls(dataEnv), dataEnv, 
                  old_data_digest = old_data_digest, masterfile = masterfile)
                do_documentation <- TRUE
            } else {
                flog.info("Some data has changed, but the DataVersion string has not been incremented or\nis less than the version in DATADIGEST \nUpdate the DataVersion string to be greater than ", 
                  old_data_digest$DataVersion, " in the DESCRIPTION file\nand re-run R CMD DataPackageR.")
            }
        } else {
            .save_data(new_data_digest, pkg_description, ls(dataEnv), dataEnv)
            do_documentation <- TRUE
        ...
        if (do_documentation) {
            ...
            if (!file.exists(file.path(target, "documentation.R"))) 
                .doc_autogen(basename(pkg_dir), ds2kp = ls(dataEnv), env = dataEnv, path = target)
            
            # parse documentation
            doc_parsed = .parseDocumentation(file.path(target, "documentation.R"))
            .identify_missing_docs = function(environment = NULL, description = NULL, 
                docs = NULL) {
            ...
            missing_doc_for_autodoc = .identify_missing_docs(dataEnv, pkg_description, 
                doc_parsed)
            if (length(missing_doc_for_autodoc) != 0) {
                tmptarget = tempdir()
                .doc_autogen(basename(pkg_dir), ds2kp = missing_doc_for_autodoc, env = dataEnv, 
                  path = tmptarget, name = "missing_doc.R")
                ...
                docfile <- file(file.path(target, paste0("documentation", ".R")), 
                  open = "w")
            ...
            # Partial build if enabled=FALSE for any file We've disabled an object but don't
            # want to overwrite its documentation or remove it The new approach just builds
            # all the docs independent of what's enabled.
            # if(!all(unlist(map(ymlconf[['configuration']][['files']],'enabled')))){
            # old_doc_file = list.files(file.path(pkg_dir,'R'),full.names =
            # TRUE,paste0(pkg_description$Package,'.R')) if (length(old_doc_file) != 0) { if
            # (file.exists(old_doc_file)) { old_docs = .parseDocumentation(old_doc_file)
            # not_built = setdiff(setdiff(names(old_docs), ls(dataEnv)),
            # pkg_description$Package) #remove all but not_built from old_docs merged_docs =
            # .mergeDocumentation(old = old_docs[not_built], new = doc_parsed) save_docs =
            # do.call(c, merged_docs) } } else { save_docs = do.call(c,doc_parsed) } }else{
            save_docs = do.call(c, doc_parsed)
            # }
            docfile <- file(file.path("R", pattern = paste0(pkg_description$Package, 
                ".R")), open = "w")
            sapply(save_docs, function(x) {
                writeLines(text = x, con = docfile)
            })
            close(docfile)
            flog.info(paste0("Copied documentation to ", file.path("R", paste0(pkg_description$Package, 
                ".R"))))
...
.vignettesFromPPFiles <- function(dir = NULL) {
    ...
    # browser()
    try(dir.create(file.path(pkg$path,"inst/doc"),showWarnings = FALSE), silent = TRUE)
    # TODO maybe copy only the files that have both html and Rmd.
    rmdfiles_for_vignettes = list.files(path = file.path(pkg$path, "data-raw"), pattern = "Rmd$", 
        full.names = TRUE, recursive = FALSE)
    htmlfiles_for_vignettes = list.files(path = file.path(pkg$path, "inst/extdata/Logfiles"), 
        pattern = "html$", full.names = TRUE, recursive = FALSE)
    purrr::map(htmlfiles_for_vignettes, function(x) file.copy(x, file.path(pkg$path, "inst/doc", 
        basename(x)), overwrite = TRUE))
    capture.output(purrr::map(rmdfiles_for_vignettes, function(x) file.copy(x, file.path(pkg$path, 
        "vignettes", basename(x)), overwrite = TRUE)))
    vignettes_to_process = list.files(path = file.path(pkg$path, "vignettes"), pattern = "Rmd$", 
        full.names = TRUE, recursive = FALSE)
    ...
    for (i in vignettes_to_process) {
        ...
        writeLines(write_me_out[[i]], con = file.path(pkg$path,"inst/doc", basename(i)))
    }
}
