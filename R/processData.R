    ...
    if (!file.exists(target)) {
      flog.fatal(paste0("Directory ", target, " doesn't exist."))
        ...
        for (i in seq_along(r_files)) {
            ...
            flog.info(paste0("Processing ", i, " of ",
        ...
        if (do_documentation) {
            ...
            close(docfile)
            flog.info(
              paste0("Copied documentation to ",
...
.ppfiles_mkvignettes <- function(dir = NULL) {
    ...
    rmdfiles_for_vignettes <-
      list.files(path = file.path(pkg$path, "data-raw"),
                 pattern = "Rmd$",
                 full.names = TRUE,
                 recursive = FALSE)
    htmlfiles_for_vignettes <-
      list.files(path = file.path(pkg$path,"inst/extdata/Logfiles"),
                 pattern = "html$",
                 full.names = TRUE,
                 recursive = FALSE)
    purrr::map(htmlfiles_for_vignettes,
               function(x) {
                 file.copy(x,
                           file.path(pkg$path,
                                     "inst/doc",
    ...
    capture.output(purrr::map(rmdfiles_for_vignettes,
                              function(x) {
                                file.copy(x,
                                          file.path(pkg$path,
                                                    "vignettes",
    ...
    vignettes_to_process <- list.files(path = file.path(pkg$path, "vignettes"),
                                       pattern = "Rmd$",
                                       full.names = TRUE,
                                       recursive = FALSE)
